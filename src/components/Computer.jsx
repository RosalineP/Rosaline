...
export const Computer = () => {
    ...
    const onStart = (bound, space) => {
        ...
        let line7 = range(-6, 7,2).map(i => [centerX + i*u1, centerY-u1*12]);
        let line6 = [-3, 15].map(i => [centerX + i*u1, centerY-u1*10]);
        let line5 = [-3, 1, 11, 15].map(i => [centerX + i*u1, centerY-u1*8]);
        let line4 = [-3, 15].map(i => [centerX + i*u1, centerY-u1*6]);
        let line3 = [-3, 15].map(i => [centerX + i*u1, centerY-u1*4]);
        let line2 = [-3, 15].map(i => [centerX + i*u1, centerY-u1*2]);
        let line1 = range(-6, 7,2).map(i => [centerX + i*u1, centerY]);
        ...
        if (totality) {
            // if (Circle.withinBound(Circle.fromCenter(space.center, space.size.minValue().value/2), space.pointer)) {
                ...
                for (let i=0; i<totality.length; i++) {
                    let d = totality[i].$subtract( space.pointer );
